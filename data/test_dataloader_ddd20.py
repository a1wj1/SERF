...
class DDD20_Data(Dataset):
    def __init__(self, root, csv_file, config):
        ...
        self.transform_enhance_ori = T.Compose([T.ToTensor(),
                                                T.Resize((256, 256))
                                                ])
    ...
    def __getitem__(self, index):
        ...
        data['dvs_filename'] = self.camera_csv['DVS_filename'].iloc[index][1:-1]
        data['aps_filename'] = self.camera_csv['APS_filename'].iloc[index][1:-1]
        ...
        for t, front_img in enumerate(self.camera_csv['APS_filename'].iloc[index][1:-1].split(",")):
            ...
            data['rgb_ori'] = self.transform_enhance_ori(np.array(Image.open(self.root + rgb_path).convert('RGB')))
        ...
        for t, front_dvs in enumerate(self.camera_csv['DVS_filename'].iloc[index][1:-1].split(",")):
            ...
            data['dvs_ori'] = self.transform_enhance_ori(np.array(Image.open(self.root + dvs_path).convert('RGB')))
